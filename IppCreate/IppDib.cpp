...
IppDib::IppDib()
	: m_nWidth(0), m_nHeight(0), m_nBitCount(0), m_nDibSize(0), m_pDib(NULL) //디폴트 생성자
{
}
...
{
	const char* ext = strrchr(filename, '.'); // 문자열 중 특정 문자가 있는 마지막 위치의 포인터를 반환(확장자 추출)
	if (!_strcmpi(ext, ".bmp")) // 대소문자 구분 없이 비교
...
BOOL IppDib::LoadBMP(const char* filename)
{
	...
	if (m_nBitCount == 24)
		m_nDibSize = sizeof(BITMAPINFOHEADER) + dwSizeImage; // 24비트 트루 컬러 비트맵의 경우
	else
		m_nDibSize = sizeof(BITMAPINFOHEADER) + sizeof(RGBQUAD) * (1 << m_nBitCount) + dwSizeImage; // 팔레트가 있는 경우(그레이 스케일 비트맵)
	...
	// 파일로부터 Packed-DIB 크기만큼을 읽기(BMP 파일에서 파일 헤더를 제외한 나머지 부분을 읽음)
...
void IppDib::Draw(HDC hdc, int dx, int dy) //비트맵 원본 크기 그대로 출력
{
	if (m_pDib == NULL)
		return;
...
void IppDib::Draw(HDC hdc, int dx, int dy, int dw, int dh, DWORD dwRop) //비트맵의 원본 전체를 확대 또는 축소하여 출력
{
	Draw(hdc, dx, dy, dw, dh, 0, 0, m_nWidth, m_nHeight, dwRop);
}

void IppDib::Draw(HDC hdc, int dx, int dy, int dw, int dh,
	int sx, int sy, int sw, int sh, DWORD dwRop) // 비트맵의 일부 영역을 지정한 크기로 출력(원본의 일부 영역을 확대 또는 축소하여 출력)
{
	if (m_pDib == NULL)
		return;
