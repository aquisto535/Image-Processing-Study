...
void IppBrightness(IppByteImage& img, int n)
{
	...
	for (int i = 0; i < size; i++)
	{
		p[i] = limit(p[i] + n); // limit 함수는 0~255 사이의 값을 보장하는 함수
	}
}
...
void IppContrast(IppByteImage& img, int n)
{
	...
	for (int i = 0; i < size; i++)
	{
		p[i] = static_cast<BYTE>(limit(p[i] + (p[i] - 128) * n / 100)); // contrast 조정 공식
	}
}
...
void IppGammaCorrection(IppByteImage& img, float gamma)
{
	...
	float gama_table[256] = {0,};

	for (int i = 0; i < 256; i++)
	{
		gama_table[i] = pow((i / 255.f), inv_Gamma);
	}
