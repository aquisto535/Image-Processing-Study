...
BOOL CGammaCorrectionDlg::OnInitDialog()
{
	...
	m_sliderGamma.SetTicFreq(20); // 슬라이더 눈금 간격 설정
	m_sliderGamma.SetPageSize(20); // 페이지 크기 설정
	m_sliderGamma.SetPos(static_cast<int>(m_fGamma * 50)); // 슬라이더 위치 설정
...
void CGammaCorrectionDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	if (m_sliderGamma.GetSafeHwnd() == pScrollBar->GetSafeHwnd())
	{
		...
		m_fGamma = pos / 50.0f; // 슬라이더 위치를 0.20 ~ 5.00 범위로 변환
		UpdateData(FALSE); // 대화 상자에 값을 업데이트
	}
...
void CGammaCorrectionDlg::OnEnChangeGammaEdit()
{
	...
	m_sliderGamma.SetPos(static_cast<int>(m_fGamma * 50)); // 슬라이더 위치 업데이트
}
